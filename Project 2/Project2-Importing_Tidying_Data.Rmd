...
| 169056301  | Chi To        | Ng        | ngxx6301\@mylaurier.ca  |
| 210261810  | Nguyen Thu An | Le        | lexx1810\@mylaurier.ca  |
| 169074296  | Ava           | Ahmed     | ahme4296\@mylaurier.ca  |
|            |               |           |                         |
|            |               |           |                         |
...
(atl_cyclone_2022_update3 <- atl_cyclone_2022_update2 |>
  ...
  ungroup() |>
  select(BasinNumberYear, Name, Entries, `1`, `2`, `3`, everything(), -group)
)
```
...
(atl_cyclone_2022_update4 <- atl_cyclone_2022_update3 |>
  select(-Entries) |>  # Due to the sanity check, `Entries` column is redundant 
   separate(BasinNumberYear, into = c("Basin", "Number", "NameYear"), sep = c(2, 4)) |>
   separate(`1`, into = c("ObservYear", "Month", "Day"), sep = c(4, 6)) |>
   select(Basin, Number, NameYear, Name, ObservYear, Month, Day, everything())
...
(atl_cyclone_2022_update5 <- atl_cyclone_2022_update4 |>
   rename(Identifier = `3`) |>
   separate(`2`, into = c("Hour", "Minute"), sep = c(2)) |>
   select(Basin, Number, NameYear, Name, ObservYear, Month, Day, Hour, Minute, Identifier, everything())
)
```
...
(atl_cyclone_2022_tidy <- atl_cyclone_2022_update5 |>
   mutate(across(c(`NameYear`, `ObservYear`, `Month`, `Day`, `Hour`, `Minute`, `Number`), as.integer)) |>
   mutate(across(14:28, as.double))
)
```
...
```{r error=TRUE, tags=c()}
(colorbrewer_tibble <- tibble(palettes = list(colorbrewer_raw))
)
```
...
(colorbrewer_data <- colorbrewer_update3 |>
    separate(palettes, into = c("red", "green", "blue"), sep = ",") |>
    mutate(
    red = as.character(str_extract(red, "\\d+")),
    green = as.character(str_extract(green, "\\d+")),
    blue = as.character(str_extract(blue, "\\d+"))
    ) |>
    select(palette_name, palette_number, color_seq, red, green, blue)
)
```
...
(xandr_raw <- xandr_address |>
    read_csv(col_types = cols(
                        `Data Provider Name` = col_character(),
                        `Data Provider ID` = col_double(),
                        `Segment ID` = col_double(),
                        `Segment Name` = col_character()
                      )) |>
    rename('data_provider_name' = 'Data Provider Name',
...
(xandr_providers <- xandr_raw |>
  summarise(.by= data_provider_name,
            provided = n()) |>
...
providers_labels <-
  labs(           
    x = "Number of records provided by a data provider",
    y = "Number of data providers",
    ...
    caption = "Data from Xandr"
  )
(providers_plot <- xandr_providers |> 
  ggplot(mapping = aes(x = provided)) +
...
(xandr_update1 <- xandr_raw |>
  summarize(.by = c(data_provider_name, segment_name),
            segment_id = min(segment_id,na.rm=TRUE)) |>
...
(retail_visit_target_sep <- retail_visit_target |>
  separate_wider_delim(segment_name, delim = ">", names = c("level_1", "level_2", "level_3", "level_4", "level_5", "level_6", "level_7"), too_few = "align_start", too_many = "merge") |>
  mutate(
...
(retail_target <- xandr_update1 |> filter(str_detect(segment_name, regex(keywords, ignore_case = TRUE)))
)
```
...
(entertain_target <- xandr_update1 |>
    filter(str_detect(segment_name, regex(keywords, ignore_case = TRUE)))
)
```
